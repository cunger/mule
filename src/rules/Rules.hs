...
relation2relation s1 s2    (Branch (Ident "apply_V2") m [ Leaf (Ident s)  m', np ])
  | s == s1             = [ Branch (Ident "apply_V2") m [ Leaf (Ident s2) m', np ] ]
  | otherwise           = []
...
relation2predicateE s1 s2    (Branch (Ident "apply_V2") m [ Leaf (Ident s)  m', _ ])
  | s == s1               = [ Branch (Ident "apply_V2") m [ Leaf (Ident s2) m', Leaf (Ident "somebody") m ] ]
  | otherwise             = []
...
relation2predicateX  s1 s2    (Branch (Ident "apply_V2") m [ Leaf (Ident s)  m', _ ])
  | s == s1                = [ Branch (Ident "apply_V2") m [ Leaf (Ident s2) m', Leaf (Ident "something") m ] ]
  | otherwise              = []
...
modAP2modAP s_ap1 s_ap2 (Branch (Ident "modify_AP_CN") m [ Leaf (Ident s)     m', cn ])
  | s == s_ap1       = [ Branch (Ident "modify_AP_CN") m [ Leaf (Ident s_ap2) m', cn ] ]
  | otherwise        = []
modAP2modAP _ _ _    = []
...
modPrepNP2modPrepNP s_prep s_np1 s_np2 (Branch (Ident "apply_Prep") m [ Leaf (Ident s) m1, Leaf (Ident s1) m2 ])
  | (s_prep == s) && (s1 == s_np1) = [ Branch (Ident "apply_Prep") m [ Leaf (Ident s) m1, Leaf (Ident s_np2) m2 ] ]
  | otherwise               = []
modPrepNP2modPrepNP _ _ _ _ = []

-- CN Prep NP -> AP CN
modPrepNP2modAP :: String -> String -> String -> Rule
modPrepNP2modAP s_prep s_np s_ap  (Branch (Ident "modify_Adv_CN") m [ Branch (Ident "apply_Prep") m' [ Leaf (Ident s1) m1, Leaf (Ident s2) m2 ], cn ])
  | s1 == s_prep && s2 == s_np = [ Branch (Ident "modify_AP_CN")  m [ Leaf (Ident s_ap) m', cn ] ]
  | otherwise           = []
modPrepNP2modAP _ _ _ _ = []

-- AP CN -> CN Prep NP
modAP2modPrepNP :: String -> String -> String -> Rule
modAP2modPrepNP s_ap s_prep s_np (Branch (Ident "modify_AP_CN")  m [ Leaf (Ident s) m', cn ])
  | s == s_ap                 = [ Branch (Ident "modify_Adv_CN") m [ Branch (Ident "apply_Prep") m' [ Leaf (Ident s_prep) m', Leaf (Ident s_np) m' ], cn ] ]
  | otherwise                 = []
